    ...
    <>
      <header className={`${overlay ? 'absolute' : 'relative'} inset-x-0 top-0 z-50 border-b ${desktopTheme}`}>
        <div className={`${overlay ? 'bg-gradient-to-b from-ink/65 to-ink/10' : ''}`}>
          <div className="editorial-container hidden h-28 grid-cols-[1fr_auto_1fr] items-center lg:grid">
          ...
          <div className="hidden h-14 border-t border-current/15 lg:block"><nav className="editorial-container flex h-full items-center justify-center gap-9 text-[10px] font-semibold uppercase tracking-editorial">{categories.map((category) => <Link key={category.slug} href={`/categories/${category.slug}`} className="transition hover:text-brand">{category.name}</Link>)}<Link href="/categories/all">All Reports</Link></nav></div>

          <div className="flex h-20 items-center justify-between px-5 sm:px-8 lg:hidden">
            <button onClick={() => setMenuOpen(true)} aria-label="Open navigation" aria-expanded={menuOpen}><Menu /></button>
            <Link href="/" aria-label="Global Luxury Reporter home" className="relative h-12 w-12 overflow-hidden"><Image src="/GLR-Logo-dp.png" alt="Global Luxury Reporter" fill priority sizes="48px" className="object-cover" /></Link>
            <button onClick={() => setSearchOpen(true)} aria-label="Open search"><Search size={21} /></button>
          </div>
        </div>
      </header>

      <div className={`fixed inset-0 z-[70] bg-ink text-white transition duration-500 lg:hidden ${menuOpen ? 'visible opacity-100' : 'invisible opacity-0'}`} aria-hidden={!menuOpen}>
