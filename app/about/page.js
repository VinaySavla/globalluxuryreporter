    ...
    <SiteShell overlayHeader>
      <main>
        ...
        <section className="editorial-container py-24 lg:py-32">
          ...
          <div className="mt-16 grid auto-rows-[220px] grid-cols-2 gap-4 md:auto-rows-[260px] md:grid-cols-4 lg:gap-6">
            {categories.slice(0, 5).map((category, index) => <div key={category.slug} className={`group relative overflow-hidden ${index === 0 || index === 2 ? 'row-span-2' : ''} ${index === 1 ? 'col-span-2' : ''}`}><Image src={category.cover_image} alt="" fill sizes="(max-width: 768px) 50vw, 25vw" className="object-cover transition duration-700 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-ink/70 to-transparent" /><h3 className="absolute bottom-5 left-6 font-serif text-2xl text-white sm:text-3xl">{category.name.replace(' & Style', '')}</h3></div>)}
          </div>
        </section>
