    ...
    <SiteShell overlayHeader>
      <main>
        ...
        <section className="editorial-container py-24 lg:py-32">
          ...
          <div className="mt-12 grid grid-cols-2 gap-3 sm:mt-16 sm:gap-5 md:grid-cols-3 lg:gap-6">
            {categories.slice(0, 6).map((category) => <div key={category.slug} className="group relative aspect-[4/5] overflow-hidden"><Image src={category.cover_image} alt={`${category.name} editorial`} fill sizes="(max-width: 768px) 50vw, 33vw" className="object-cover transition duration-700 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-ink/70 to-transparent" /><h3 className="absolute bottom-4 left-4 right-4 font-serif text-xl text-white sm:bottom-5 sm:left-6 sm:text-3xl">{category.name.replace(' & Style', '')}</h3></div>)}
          </div>
        </section>
